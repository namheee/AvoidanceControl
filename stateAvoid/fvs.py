...
def _restricted_candidate_list(directed_graph, alpha):
    ...
    for node in directed_graph.nodes():
        in_deg = directed_graph.in_degree(node)
        out_deg = directed_graph.out_degree(node)
        G_values[node] = in_deg * out_deg

    G = list(G_values.values())
    if not G:
        return []
    ...
    RCL = [node for node, g_val in G_values.items() if g_val >= threshold]
...
def _construct_greedy_randomized_solution(directed_graph, alpha, S):
    ...
    while len(reduced_graph) > 0:
        ...
        RCL_list = list(RCL) # 리스트로 변환.
        s = np.random.choice(RCL_list)
